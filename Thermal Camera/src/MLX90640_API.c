...
int MLX90640_SetSubPageNumber(uint16_t subPage);
int MLX90640_IsSubPageRepeat(void);
...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    { 
    	...
    	if (statusRegister&0x0001)
    	{
			...
			readSP1 = 1;
    		if(MLX90640_IsSubPageRepeat())
    		{
    			MLX90640_SetSubPageNumber(0);
    		}
...
int MLX90640_SetSubPageRepeat(uint8_t subPageEnable)
{
    uint16_t controlRegister1;
    int value;
    int error;

    error = MLX90640_I2CRead(0x33, 0x800D, 1, &controlRegister1);
    if(error == 0)
    {
    	value = (subPageEnable | 0x0001)<<3;
    	value = (controlRegister1 & 0xFFF7) | value;
    	error = MLX90640_I2CWrite(0x33, 0x800D, value);
    }
    return error;
}
...
int MLX90640_IsSubPageRepeat(void)
{
    ...
    return ((controlRegister1 & 0x0008)>>3);
}
//------------------------------------------------------------------------------
...
void MLX90640_CalculateTo(uint16_t *frameData, const paramsMLX90640 *params, float emissivity, float tr, float *result)
{
    ...
    float taTr;
    float gain;
    extern RingBuffer irDataCP0;
    extern RingBuffer irDataCP1;
    float irDataCP[2];
    float irData;
...
//------------------------- To calculation -------------------------------------    
    ...
    rb_addValue(&irDataCP0, (frameData[776]>32767 ? ((float)(frameData[776]-65536)* gain) : ((float)frameData[776]))* gain);  	//CP (SP0)
    rb_addValue(&irDataCP0, (frameData[808]>32767 ? ((float)(frameData[808]-65536)* gain): ((float)frameData[776]))* gain);
    //irDataCP[0] = frameData[776];	//CP (SP0)
    //irDataCP[1] = frameData[808];	//CP (SP1)
    ...
    /* -- 11.2.2.6.2 -- */
    irDataCP[0] = irDataCP0.mvgAvg - params->cpOffset[0] * (1 + params->cpKta * (ta - 25)) * (1 + params->cpKv * (vdd - 3.3));
    if( mode == CP_Mode)
    {
        irDataCP[1] = irDataCP1.mvgAvg - params->cpOffset[1] * (1 + params->cpKta * (ta - 25)) * (1 + params->cpKv * (vdd - 3.3));
    }
    else
    {
      irDataCP[1] = irDataCP1.mvgAvg - (params->cpOffset[1] + params->ilChessC[0]) * (1 + params->cpKta * (ta - 25)) * (1 + params->cpKv * (vdd - 3.3));
    }
...
/*
            alphaCompensated = params->alpha[pixelNumber]-params->tgc*(params->cpAlpha[0]*(1-pattern) + params->cpAlpha[1]*pattern);
            alphaCompensated = alphaCompensated*(1 + params->KsTa * (ta - 25));
*/
